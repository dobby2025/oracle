/*
파일명: ex20-data-dictionary.sql

데이터 딕셔너리란?
    데이터 딕셔너리는 데이터베이스 내부의 메타데이터를 관리하는 
    시스템 테이블과 뷰의 집합입니다.
...
-- all_tables: 사용자가 접근 가능한 모든 테이블 정보 조회
SELECT owner, table_name, num_rows
FROM all_tables 
WHERE owner NOT IN ('SYS','SYSTEM');

-- all_indexes : 접근가능한 모든 인덱스 정보 조회
SELECT owner, index_name, table_name
FROM all_indexes
WHERE owner NOT IN ('SYS', 'SYSTEM');

-- dba_tables: 데이터베이스의 모든테이블 정보 조회 (DBA 권한필요)
SELECT owner, table_name, num_rows
FROM dba_tables
WHERE owner = 'HR';
